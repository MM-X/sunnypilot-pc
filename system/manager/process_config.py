...
from openpilot.system.hardware import PC, TICI, RK
...
# ponytail: FILLBACK set (current) disables camerad/pandad; absent reverts to committed (enabled)
...
procs = [
  ...
  NativeProcess("camerad", "system/camerad", ["./camerad"], driverview, enabled=(not FILLBACK) and not WEBCAM),
  ...
  NativeProcess("ui", "selfdrive/ui", ["./ui"], always_run, watchdog_max_dt=(5 if (not PC or RK) else None)),
  PythonProcess("raylib_ui", "selfdrive.ui.ui", always_run, enabled=False, watchdog_max_dt=(5 if not PC else None)),
  PythonProcess("soundd", "selfdrive.ui.soundd", only_onroad, enabled=not RK),
  ...
  PythonProcess("pandad", "selfdrive.pandad.pandad", always_run, enabled=not FILLBACK),
