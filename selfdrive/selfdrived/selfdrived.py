...
from openpilot.system.hardware import RK, PC
...
FILLBACK = "FILLBACK" in os.environ
...
class SelfdriveD(CruiseHelper):
  def __init__(self, CP=None, CP_SP=None):
    ...
    self.camera_packets = ["roadCameraState"] + ([] if PC else ["wideRoadCameraState"])

    # TODO: de-couple selfdrived with card/conflate on carState without introducing controls mismatches
    self.car_state_sock = messaging.sub_sock('carState', timeout=20)

    ignore = self.sensor_packets + self.gps_packets + ['alertDebug', 'modelDataV2SP']
    # ponytail: PC has no lagd/torqued running; liveDelay/liveTorqueParameters won't update, ignore to avoid commIssue
    if PC:
      ignore += ['accelerometer', 'gyroscope','liveDelay', 'liveTorqueParameters']
    if SIMULATION:
    ...
    self.rk = Ratekeeper(50 if RK else 100, print_delay_threshold=None)
    ...
    elif HARDWARE.get_device_type() == 'tici':
      self.ignored_processes = {'dmonitoringmodeld', 'dmonitoringd'}
  ...
  def update_events(self, CS):
    ...
    for i, pandaState in enumerate(self.sm['pandaStates']):
      ...
      # ponytail: FILLBACK set (current) disables; absent reverts to committed safety_mismatch guard
      if ((not FILLBACK and safety_mismatch and self.sm.frame*DT_CTRL > 10.) or pandaState.safetyRxChecksInvalid or self.mismatch_counter >= 200):
    ...
    # ponytail: FILLBACK set (current) disables; absent reverts to committed guard
    if not FILLBACK and any((self.sm.frame - self.sm.recv_frame[s])*DT_CTRL > 10. for s in self.sensor_packets):
    ...
    if not PC and not gps_ok and self.sm['livePose'].inputsOK and (self.distance_traveled > 1500):
  ...
  def publish_selfdriveState(self, CS):
    ...
    if (self.sm.frame % int((0.5 if RK else 1.) / DT_CTRL) == 0) or (self.events.names != self.events_prev):
    ...
    if (self.sm.frame % int((0.5 if RK else 1.) / DT_CTRL) == 0) or (self.events_sp.names != self.events_sp_prev):
