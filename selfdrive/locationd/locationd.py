...
class LocationEstimator:
  ...
  def _validate_timestamp(self, t: float):
    ...
    if invalid:
      cloudlog.warning(f"Observation timestamp is older than the max rewind threshold of the filter (kf_t={kf_t:.3f} t={t:.3f} gap={kf_t - t:.3f})")
    return not invalid
  ...
  def handle_log(self, t: float, which: str, msg: capnp._DynamicStructReader) -> HandleLogResult:
    ...
    if which == "accelerometer" and msg.which() == "acceleration":
      ...
      if PC:
        # ponytail: PC has no IMU (sensord disabled); derive accel/gyro from carState
        # reference: b51be4e + 306a880
        sensor_time = t
        acc_meas = np.array([msg.aEgo, 0, -9.81])
        if np.linalg.norm(acc_meas) >= ACCEL_SANITY_CHECK:
          return HandleLogResult.INPUT_INVALID
        acc_res = self.kf.predict_and_observe(sensor_time, ObservationKind.PHONE_ACCEL, acc_meas)
        if acc_res is not None:
          _, new_x, _, new_P, _, _, (acc_err,), _, _ = acc_res
          self.observation_errors[ObservationKind.PHONE_ACCEL] = np.array(acc_err)
          self.observations[ObservationKind.PHONE_ACCEL] = acc_meas

        gyro_meas = np.array([0, 0, -msg.yawRate])
        gyro_bias = self.kf.x[States.GYRO_BIAS]
        gyro_camodo_yawrate_err = np.abs((gyro_meas[2] - gyro_bias[2]) - self.camodo_yawrate_distribution[0])
        gyro_camodo_yawrate_err_threshold = YAWRATE_CROSS_ERR_CHECK_FACTOR * self.camodo_yawrate_distribution[1]
        gyro_valid = gyro_camodo_yawrate_err < gyro_camodo_yawrate_err_threshold
        if np.linalg.norm(gyro_meas) >= ROTATION_SANITY_CHECK or not gyro_valid:
          return HandleLogResult.INPUT_INVALID
        gyro_res = self.kf.predict_and_observe(sensor_time, ObservationKind.PHONE_GYRO, gyro_meas)
        if gyro_res is not None:
          _, new_x, _, new_P, _, _, (gyro_err,), _, _ = gyro_res
          self.observation_errors[ObservationKind.PHONE_GYRO] = np.array(gyro_err)
      ...
      if PC:
        # ponytail: PC filter time is driven by carState.logMonoTime (live clock), but frame
        # timestampEof may be in a different clock domain (e.g. replay). Align to logMonoTime.
...
def main():
  ...
  # separate sensor sockets for efficiency (TICI only; PC has no IMU, accel/gyro come from carState)
  sensor_sockets = [] if PC else [messaging.sub_sock(which, timeout=20) for which in ['accelerometer', 'gyroscope']]
  ...
  critcal_services = ["cameraOdometry"] if PC else ["accelerometer", "gyroscope", "cameraOdometry"]
  ...
  while True:
    ...
    acc_msgs, gyro_msgs = ([], []) if PC else (messaging.drain_sock(sock) for sock in sensor_sockets)
    ...
    if filter_initialized:
      ...
      filter_initialized = sm.all_checks() and (True if PC else sensor_all_checks(acc_msgs, gyro_msgs, sensor_valid, sensor_recv_time, sensor_alive, SIMULATION))

    if sm.updated["cameraOdometry"]:
      critical_service_inputs_valid = all(observation_input_invalid[s] < input_invalid_threshold[s] for s in critcal_services)
      inputs_valid = sm.all_valid() and critical_service_inputs_valid
      sensors_valid = True if PC else sensor_all_checks(acc_msgs, gyro_msgs, sensor_valid, sensor_recv_time, sensor_alive, SIMULATION)
