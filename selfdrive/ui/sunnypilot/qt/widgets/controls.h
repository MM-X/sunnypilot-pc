...
public:
  MultiButtonControlSP(const QString &title, const QString &desc, const QString &icon,
                     ...
                     const bool inline_layout = false, bool advancedControl = false) : AbstractControlSP_SELECTOR(title, desc, icon, nullptr, advancedControl), button_texts(button_texts), is_inline_layout(inline_layout) {
    const QString style = R"UI0(
      QPushButton {)UI0"
#ifdef RK_BUILD
...
#ifdef RK_BUILD
      button->setMinimumWidth(minimum_button_width * 37 / 100);
#else
      button->setMinimumWidth(minimum_button_width);
#endif
      if (i == 0) button_param_layout->
#ifdef RK_BUILD
      addSpacing(1);
#else
      addSpacing(2);
...
public:
  ButtonParamControlSP(const QString &param, const QString &title, const QString &desc, const QString &icon,
                     const std::vector<QString> &button_texts, const int minimum_button_width = 225,
                     const bool inline_layout = false, bool advancedControl = false) : MultiButtonControlSP(title, desc, icon,
...
R"UI7(        color: #FFFFFF;
    ...
    label.setFixedWidth(
#ifdef RK_BUILD
      inline_layout ? 129 : 111
#else
      inline_layout ? 350 : 300
#endif
    );
