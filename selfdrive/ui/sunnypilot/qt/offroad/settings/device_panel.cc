...
DevicePanelSP::DevicePanelSP(SettingsWindowSP *parent) : DevicePanel(parent) {
  ...
  for (const auto &[id, text, param] : device_btns) {
    ...
    auto *btn = new PushButtonSP(text,
#ifdef RK_BUILD
                                 277,
#else
                                 750,
...
#ifdef RK_BUILD
  power_layout->setSpacing(9);
#else
  power_layout->setSpacing(25);
#endif

  PushButtonSP *rebootBtn = new PushButtonSP(tr("Reboot"),
#ifdef RK_BUILD
                                             277,
#else
                                             750,
  ...
  PushButtonSP *poweroffBtn = new PushButtonSP(tr("Power Off"),
#ifdef RK_BUILD
                                               277,
#else
                                               750,
...
#ifdef RK_BUILD
  power_group_layout->setSpacing(9);
#else
