...
SettingsWindowSP::SettingsWindowSP(QWidget *parent) : SettingsWindow(parent) {
  ...
  close_btn->setStyleSheet(R"UI0(
    QPushButton {)UI0"
#ifdef RK_BUILD
R"UI1(      font-size: 52px;
      padding-bottom: 7px;
      border-radius: 28px;)UI1"
#else
R"UI2(      font-size: 140px;
      padding-bottom: 20px;
      border-radius: 76px;)UI2"
#endif
R"UI3(      background-color: #292929;)UI3"
#ifdef RK_BUILD
R"UI4(      font-weight: 148;)UI4"
#else
R"UI5(      font-weight: 400;)UI5"
#endif
R"UI6(    }
    QPushButton:pressed {
...
#endif
    btn->setStyleSheet(R"UI7(
      QPushButton {)UI7"
#ifdef RK_BUILD
R"UI8(        border-radius: 7px;
        width: 148px;
        height: 36px;)UI8"
#else
R"UI9(        border-radius: 20px;
        width: 400px;
        height: 98px;)UI9"
#endif
R"UI10(        color: #bdbdbd;
        border: none;
        background: none;)UI10"
#ifdef RK_BUILD
R"UI11(        font-size: 19px;
        font-weight: 185;)UI11"
#else
R"UI12(        font-size: 50px;
        font-weight: 500;)UI12"
#endif
R"UI13(        text-align: left;)UI13"
#ifdef RK_BUILD
R"UI14(        padding-left: 8px;)UI14"
#else
R"UI15(        padding-left: 22px;)UI15"
#endif
R"UI16(      }
      QPushButton:checked {
...
R"UI20(    }
    ...
    QStackedWidget, ScrollViewSP {
      background-color: black;)UI20"
#ifdef RK_BUILD
R"UI21(      border-radius: 11px;)UI21"
#else
