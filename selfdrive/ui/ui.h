...
// ponytail: FILLBACK set (current) stock 1928x1208/2648; absent reverts to committed RK_BUILD webcam intrinsic
static inline Eigen::Matrix3f _make_fcam_intrinsic() {
  if (std::getenv("FILLBACK") != nullptr)
    return (Eigen::Matrix3f() << 2648.0, 0.0, 1928.0 / 2, 0.0, 2648.0, 1208.0 / 2, 0.0, 0.0, 1.0).finished();
#ifdef RK_BUILD
  return (Eigen::Matrix3f() << 900.0, 0.0, 1280.0 / 2, 0.0, 900.0, 720.0 / 2, 0.0, 0.0, 1.0).finished();
#else
  return (Eigen::Matrix3f() << 2648.0, 0.0, 1928.0 / 2, 0.0, 2648.0, 1208.0 / 2, 0.0, 0.0, 1.0).finished();
#endif
}
const Eigen::Matrix3f FCAM_INTRINSIC_MATRIX = _make_fcam_intrinsic();
