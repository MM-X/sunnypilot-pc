...
KeyboardLayout::KeyboardLayout(QWidget* parent, const std::vector<QVector<QString>>& layout) : QWidget(parent) {
  ...
  setStyleSheet(QString(R"UI0(
    QPushButton {)UI0"
#ifdef RK_BUILD
R"UI1(      font-size: 27px;)UI1"
#else
R"UI2(      font-size: 75px;)UI2"
#endif
R"UI3(      margin-left: %1px;
      margin-right: %1px;)UI3"
#ifdef RK_BUILD
R"UI4(      margin-top: %1px;
      margin-bottom: %1px;)UI4"
#else
R"UI5(      margin-top: %2px;
      margin-bottom: %2px;)UI5"
#endif
R"UI6(      padding: 0px;)UI6"
#ifdef RK_BUILD
R"UI7(      border-radius: 4px;)UI7"
#else
R"UI8(      border-radius: 10px;)UI8"
#endif
R"UI9(      color: #dddddd;
      background-color: #444444;
    }
  ...
  )UI9").arg(key_spacing_vertical / 2).arg(key_spacing_horizontal / 2));
}
