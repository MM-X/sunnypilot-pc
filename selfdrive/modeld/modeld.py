...
def main(demo=False):
  ...
  while True:
    ...
    if sm.frame % 60 == 0:
      # ponytail: PC has no lagd running; fallback to CP.steerActuatorDelay + 0.2 (lagd initial_lag)
      model.lat_delay = (CP.steerActuatorDelay + 0.2) if PC else get_lat_delay(params, sm["liveDelay"].lateralDelay)
    ...
    # ponytail: rockchip CL backend ~50-60ms can't hold 20Hz; drop every other frame
    # (camerad stays 20fps, run() stays 10Hz continuous so prev_feat/desire_q stay consistent)
    if RK and run_count % 2 == 1:
    ...
    if FILLBACK and model_execution_time > 0.04:
      print(f"model execution time: {model_execution_time:.3f} sec")
