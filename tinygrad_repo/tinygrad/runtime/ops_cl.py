...
class CLProgram:
  ...
  def __call__(self, *bufs:cl.cl_mem, global_size:tuple[int,int,int]=(1,1,1), local_size:tuple[int,int,int]|None=None, vals:tuple[int, ...]=(),
    ...
    # ponytail: release per-call image wrappers; enqueue retains them until the kernel completes.
    # Without this, Mali leaks a device-side image descriptor (~MBs) every frame on image2d_from_buffer args.
    for img in imgs: check(cl.clReleaseMemObject(img))
    if wait:
...
# cl_arm_import_memory: true zero-copy host import via clImportMemoryARM.
# Mali allocates a device shadow per kernel access with CL_MEM_USE_HOST_PTR (leaks ~5MB/frame);
# clImportMemoryARM(CL_IMPORT_TYPE_HOST_ARM) imports the host ptr with no shadow.
_import_fn_ptr = cl.clGetExtensionFunctionAddress(b"clImportMemoryARM")
_clImportMemoryARM = None
if _import_fn_ptr:
  _clImportMemoryARM = ctypes.CFUNCTYPE(cl.cl_mem, cl.cl_context, cl.cl_mem_flags, ctypes.c_void_p,
                                        ctypes.c_void_p, ctypes.c_size_t, ctypes.POINTER(ctypes.c_int32))(_import_fn_ptr)
  # CL_IMPORT_TYPE_ARM=0x40B2, CL_IMPORT_TYPE_HOST_ARM=0x40B3, 0=terminator; cl_import_properties_arm=intptr_t(64-bit)
  _import_host_props = (ctypes.c_int64*3)(0x40B2, 0x40B3, 0)
...
class CLAllocator(LRUAllocator['CLDevice']):
  def _alloc(self, size:int, options:BufferSpec) -> cl.cl_mem:
    if options is not None and options.external_ptr is not None:
      if _clImportMemoryARM is not None:
        return checked(_clImportMemoryARM(self.dev.context, cl.CL_MEM_READ_WRITE, ctypes.cast(_import_host_props, ctypes.c_void_p),
                                          ctypes.c_void_p(options.external_ptr), size, status := ctypes.c_int32()), status)
      # fallback: CL_MEM_USE_HOST_PTR (leaks driver shadow on Mali)
      flags = cl.CL_MEM_READ_WRITE | cl.CL_MEM_USE_HOST_PTR
      return checked(cl.clCreateBuffer(self.dev.context, flags, size, ctypes.c_void_p(options.external_ptr), status := ctypes.c_int32()), status)
